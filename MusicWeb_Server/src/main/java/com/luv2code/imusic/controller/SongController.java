...
//    }
    ...
    public ResponseEntity<String> addSong(HttpServletRequest req,
        ...
        String artist_id = artistId.trim();
        String category_id = categoriesId.trim();
        log.info(req.getParameter("artist"));
        String name = song.getName().trim();
        String introduction = song.getIntroduction().trim();
        Boolean status = song.isEnabled();     
    ...
    public ResponseEntity<Iterable<Song>> findAll() {
        Iterable<Song> findAllArtists = songService.findAll();
        return new ResponseEntity<Iterable<Song>>(findAllArtists,HttpStatus.OK);
    }
    
    @GetMapping("findall2")
    public ResponseEntity<Iterable<User>> findAll2() {
        Iterable<User> findAllArtists = userService.findAll();
        return new ResponseEntity<Iterable<User>>(findAllArtists,HttpStatus.OK);
    }
