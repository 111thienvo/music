...
class Pagination
{
    protected $_config = [
        ...
        'range'         => 5, // Số li trang bạn muốn hiển thị
        'min'           => 0, // Tham số min
    ...
    function html()
    {
        ...
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            if ($this->_config['current_page'] > 1)
            {
                $p .= '<li><a class="btn btn-secondary" href="'.$this->__link('1').'">First</a></li>';
                $p .= '<li><a class="btn btn-secondary" href="'.$this->__link($this->_config['current_page']-1).'">Back</a></li>';
            }
...
//                    $p .= '<li class="btn btn-info" ><span class="btn btn-info">'.$i.'</span></li>';
                    $p .= '<li> <a class="btn btn-danger"> '.$i.'</a></li>';
                }
                else{
                    $p .= '<li><a class="btn btn-secondary" href="'.$this->__link($i).'">'.$i.'</a></li>';
                }
            }
            // Nút last và next
            if ($this->_config['current_page'] < $this->_config['total_page'])
            {
                $p .= '<li><a class="btn btn-secondary" href="'.$this->__link($this->_config['current_page'] + 1).'">Next</a></li>';
                $p .= '<li><a class="btn btn-secondary" href="'.$this->__link($this->_config['total_page']).'">Last</a></li>';
            }
            $p .= '</ul>';
        }
        return $p;
